...
function Library(props: Props) {
  return (
      ...
      <div className="flex flex-row items-center justify-between font-author w-full">
          <div className="flex items-center space-x-2">
        <div className="gradientBorder overflow-hidden rounded-2xl duration-300 hover:scale-105 p-[2.5px]">
          <div className="flex h-16 w-16 items-center rounded-2xl bg-white p-2 ">
            <Image src={props.libraryLogo} height="100%" width="100%" />
          </div>
        </div>
          {/* Title */}
        <h1 className="text-center text-xl text-white/90 ">
          {props.libraryName}
        </h1>
      </div>
         {/* Button */}
     <div className="gradientBorder group p-[2px] font-author text-lg">
          <button className="group relative inline-flex items-center justify-start overflow-hidden rounded-md bg-[#171717] px-3 py-1 font-normal">
            <span className="absolute left-0 top-0 h-32 w-32 translate-x-12 -translate-y-2 rotate-45 bg-gradient-to-tr from-[#50afeb] to-[#cd0789] from opacity-[0%]"></span>
            <span className="absolute top-0 left-0 -mt-1 h-48 w-48 -translate-x-56 -translate-y-24 rotate-45 bg-gradient-to-tr from-[#50afeb] to-[#cd0789] opacity-100 transition-all duration-500 ease-in-out group-hover:-translate-x-8"></span>
